...
class TestWalletAcquireCertificate:
    ...
    Reference: toolbox/ts-wallet-toolbox/test/Wallet/certificate/acquireCertificate.test.ts
    """
    ...
    async def test_acquirecertificate_listcertificate_provecertificate(self) -> None:
        ...
        list_result = await wallet.list_certificates({
            "certifiers": [cert_data["certifier"]],
            "types": []
        })
        ...
        verifiable_cert = _create_verifiable_certificate(
            lc, prove_result["keyringForVerifier"]
        )
    ...
    async def test_privileged_acquirecertificate_listcertificate_provecertificate(self) -> None:
        ...
        decrypted = await _decrypt_fields(
            verifiable_cert, wallet, privileged=True, privileged_reason="more cheese"
        )
        assert decrypted["name"] == "Alice"
...
def _create_verifiable_certificate(cert: dict, keyring: dict):
