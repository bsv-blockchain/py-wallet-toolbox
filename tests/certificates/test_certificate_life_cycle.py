...
Reference: toolbox/ts-wallet-toolbox/src/sdk/__test/CertificateLifeCycle.test.ts
"""
...
def make_sample_cert(
    subject_root_key_hex: str = None,
    certifier_key_hex: str = None,
    verifier_key_hex: str = None
):
    ...
    verifier = PrivateKey.from_string(verifier_key_hex) if verifier_key_hex else PrivateKey.from_random()
...
class TestCertificateLifeCycle:
    ...
    async def test_complete_flow_mastercertificate_and_verifiablecertificate(self) -> None:
        ...
        r1 = await MasterCertificate.create_certificate_fields(
            certifier_wallet,
            subject.to_public_key().to_string(),
            cert.fields
        )
        ...
        r2 = await MasterCertificate.decrypt_fields(
            subject_wallet,
            r1["masterKeyring"],
            signed_cert.fields,
