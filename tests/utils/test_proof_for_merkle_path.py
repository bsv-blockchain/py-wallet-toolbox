        ...
        for tc in test_cases:
            ...
            if tc["want_err"]:
                with pytest.raises(Exception):
                    convert_tsc_proof_to_merkle_path(tc["txid"], tc["index"], tc["nodes"], tc["block_height"])
            else:
                mp = convert_tsc_proof_to_merkle_path(tc["txid"], tc["index"], tc["nodes"], tc["block_height"])

                # Then
                assert mp is not None, f"Test '{tc['name']}' failed"
                assert mp["block_height"] == tc["block_height"]

                if tc.get("check_duplicate"):
                    assert (
