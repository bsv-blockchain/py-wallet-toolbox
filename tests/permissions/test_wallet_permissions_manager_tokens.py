...
Reference: toolbox/ts-wallet-toolbox/src/__tests/WalletPermissionsManager.tokens.test.ts
"""
...
class TestWalletPermissionsManagerTokens:
    ...
    @pytest.mark.skipif(not IMPORTS_AVAILABLE, reason="Waiting for WalletPermissionsManager implementation")
    @pytest.mark.asyncio
    async def test_should_spend_the_old_token_input_and_create_a_new_protocol_token_output_with_updated_expiry(self) -> None:
    ...
    async def test_should_allow_updating_the_authorizedamount_in_dsap_renewal(self) -> None:
        ...
        previous_token = {
            "txid": "olddsaptxid",
            "outputIndex": 0,
            "authorizedAmount": 10000  # Old amount
        }
    ...
    async def test_should_remove_the_old_token_from_listing_after_revocation(self) -> None:
        ...
        mock_underlying_wallet.list_outputs = AsyncMock(return_value={
            "outputs": []  # Empty after revocation
        })
        manager = WalletPermissionsManager(
            underlying_wallet=mock_underlying_wallet,
            admin_originator="admin.domain.com"
