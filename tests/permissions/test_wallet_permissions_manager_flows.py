...
Reference: toolbox/ts-wallet-toolbox/src/__tests/WalletPermissionsManager.flows.test.ts
"""
...
class TestWalletPermissionsManagerFlows:
    ...
    async def test_should_coalesce_parallel_requests_for_the_same_resource_into_a_single_user_prompt(self) -> None:
        ...
        # When - make two parallel calls for same resource
        call_a = asyncio.create_task(manager.ensure_protocol_permission({
        ...
        call_b = asyncio.create_task(manager.ensure_protocol_permission({
            "originator": "example.com",
            "privileged": False,
            "protocolID": [1, "someproto"],
            "counterparty": "self",
        ...
        # Deny the request
        await manager.deny_permission(request_id)
    ...
    async def test_should_generate_two_distinct_user_prompts_for_two_different_permission_requests(self) -> None:
        ...
        p_call = asyncio.create_task(manager.ensure_protocol_permission({
            "originator": "example.com",
            "privileged": False,
        ...
        b_call = asyncio.create_task(manager.ensure_basket_access({
            "originator": "example.com",
            "basket": "some-basket",
        ...
        await manager.deny_permission(p_req_id)
        
        # Deny basket request
        b_req_id = basket_request_cb.call_args[0][0]["requestID"]
        await manager.deny_permission(b_req_id)
    ...
    @pytest.mark.skipif(not IMPORTS_AVAILABLE, reason="Waiting for WalletPermissionsManager implementation")
    @pytest.mark.asyncio
    async def test_should_resolve_all_parallel_requests_when_permission_is_granted_referencing_the_same_requestid(self) -> None:
        ...
        # When - parallel calls
        promise_a = asyncio.create_task(manager.ensure_protocol_permission({
            "originator": "example.com",
            "privileged": False,
        ...
        promise_b = asyncio.create_task(manager.ensure_protocol_permission({
            "originator": "example.com",
            "privileged": False,
            "protocolID": [1, "proto-X"],
            "counterparty": "anyone",
    ...
    async def test_should_reject_only_the_matching_request_queue_on_deny_if_requestid_is_specified(self) -> None:
        ...
        # When - Resource 1
        p1_promise = asyncio.create_task(manager.ensure_protocol_permission({
        ...
        # Resource 2
        p2_promise = asyncio.create_task(manager.ensure_protocol_permission({
        ...
        await manager.deny_permission(p2_req_id)
        with pytest.raises(ValueError, match="Permission denied"):
            await p2_promise
        ...
        await manager.deny_permission(p1_req_id)
        with pytest.raises(ValueError, match="Permission denied"):
            await p1_promise
        assert len(active_requests) == 0
    ...
    async def test_should_not_create_a_token_if_ephemeral_true_so_subsequent_calls_re_trigger_the_request(self) -> None:
        ...
        # When - call 1
        p_call1 = asyncio.create_task(manager.ensure_protocol_permission({
        ...
        await manager.grant_permission({"requestID": req_id1, "ephemeral": True})
        await p_call1
        
        # Then - call 2 should re-trigger
        p_call2 = asyncio.create_task(manager.ensure_protocol_permission({
            "originator": "appdomain.com",
            "privileged": False,
            "protocolID": [1, "ephemeral-proto"],
            "counterparty": "self",
        ...
        await manager.grant_permission({"requestID": req_id2, "ephemeral": True})
        await p_call2
    
    @pytest.mark.skipif(not IMPORTS_AVAILABLE, reason="Waiting for WalletPermissionsManager implementation")
    @pytest.mark.asyncio
    async def test_should_create_a_token_if_ephemeral_false_so_subsequent_calls_do_not_re_trigger_if_unexpired(self) -> None:
        ...
        await manager.grant_permission({"requestID": req_id, "ephemeral": False})
        await p_call1
    ...
    @pytest.mark.skipif(not IMPORTS_AVAILABLE, reason="Waiting for WalletPermissionsManager implementation")
    @pytest.mark.asyncio
    async def test_should_handle_renewal_if_the_found_token_is_expired_passing_previoustoken_in_the_request(self) -> None:
        ...
        await manager.grant_permission({"requestID": req_id, "ephemeral": True})
        await p_call
