        ...
        try:
            ...
            for vin, tx_input in enumerate(tx.inputs):
                ...
                if not unlock_script or len(unlock_script) == 0:
                    logger.warning(
                        f"Transaction {tx.txid()} has input {vin} without unlocking script"
                    )
                    return False
                
                # Log debug info
                logger.debug(
                    f"Transaction {tx.txid()} input {vin} unlocking script: {unlock_script}"
                )
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as pool:
                    is_valid = loop.run_in_executor(
                        pool, lambda: asyncio.run(_async_verify())
                    )
            except RuntimeError:
                # No running loop
