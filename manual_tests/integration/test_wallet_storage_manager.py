...
Reference: toolbox/ts-wallet-toolbox/src/storage/__test/WalletStorageManager.test.ts
"""
    ...
    async def test_runasreader_runaswriter_runassync_interlock_correctly(self) -> None:
        ...
        for r in result:
            overlaps = [
                r2 for r2 in result
                if r2.i != r.i and
                   (r2.t != "reader" or r.t != "reader") and
                   r.start > r2.start and r.start < r2.end
            ]
            if len(overlaps) > 0:
...
# Helper function stubs (to be implemented with WalletStorageManager)
import asyncio


async def create_sqlite_test_storage(database_name: str):
